...
from OffIdentity.admin import UserCreationForm
...
from infrastructure.forms.user import UserCreateForm
...
class OffIdentityLightUserCreationForm(UserCreateForm):
    @transaction.atomic
...
class OffIdentityCreationForm(OffIdentityLightUserCreationForm):
    class Meta:
        model = OffIdentity
        fields = ['key']
...
class OffIdentityCreate(SuccessMessageMixin, RedirectNextMixin, CreateView, GenericNavigationMixin):
    model = OffIdentity
    template_name = 'OffIdentity/create_identity.html'
    contextbuilder = None
    success_message = _('identity_is_created')
    email_template_name = "OffIdentity/emails/registration_email_template.html"
    html_email_template_name = "OffIdentity/emails/registration_html_email_template.html"
    subject_template_name = "OffIdentity/emails/registration_email_subject_template.txt"
    
    def get(self, request, *args, **kwargs):
        self.contextbuilder = ContextBuilder(request)
        try:
            key = self.kwargs['key'].lower()
            identity = OffIdentity.objects.get(key=key)
            return redirect('OffIdentity:update_key', off_id=identity.key)
        except OffIdentity.DoesNotExist:
            pass
        except KeyError:
            pass
    ...
    def get_form_class(self):
        if self._is_anonymous_request(self.request):
            return OffIdentityLightUserCreationForm
        return OffIdentityCreationForm
